...
use crate::protocol::{
    BatchAck, EntityUpdate, InventoryDelta, InventoryUpdate, 
    OutboxBatch, SyncMessage,
};
...
pub trait CloudUplinkCallback: Send + Sync {
    ...
    async fn sync_entity(&self, entity_type: &str, entity_id: &str, payload: &str) -> SyncResult<()>;
    
    /// Called when an inventory delta needs to be recorded.
    async fn record_inventory_delta(&self, product_id: &str, sku: &str, delta: i32, source: &str) -> SyncResult<()>;
}
...
impl CloudUplinkCallback for NoOpCloudUplink {
    async fn sync_entity(&self, _entity_type: &str, _entity_id: &str, _payload: &str) -> SyncResult<()> {
        Ok(())
    }
    
    async fn record_inventory_delta(&self, _product_id: &str, _sku: &str, _delta: i32, _source: &str) -> SyncResult<()> {
        Ok(())
    }
}
...
impl HubCore {
    ...
    pub async fn register_client(
        ...
        store_id: &str,
        addr: SocketAddr,
    ) -> SyncResult<(mpsc::Sender<String>, mpsc::Receiver<String>, broadcast::Receiver<String>)> {
        // Validate store ID
    ...
    pub async fn send_to_client(&self, device_id: &str, message: &str) -> SyncResult<()> {
        ...
        if let Some(client) = clients.get(device_id) {
            client.tx.send(message.to_string()).await
    ...
    pub async fn handle_message(
        ...
        match message {
            ...
            SyncMessage::OutboxBatch(batch) => {
                self.handle_outbox_batch(device_id, batch).await
            }
    ...
    async fn handle_inventory_delta(
        ...
        if let Some(ref uplink) = self.cloud_uplink {
            if let Err(e) = uplink.record_inventory_delta(
                &delta.product_id,
                &delta.sku,
    ...
    async fn handle_outbox_batch(
        ...
        for entity in &batch.entities {
            ...
            if let Some(ref uplink) = self.cloud_uplink {
                if let Err(e) = uplink.sync_entity(
                    &entity.entity_type,
                    &entity.entity_id,
                    &entity.payload,
                ).await {
...
mod tests {
    ...
    async fn test_client_registration() {
        ...
        let result = hub.register_client("device-1", "Test Device", "test-store", addr).await;
        assert!(result.is_ok());
